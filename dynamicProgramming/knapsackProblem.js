...
const bag = (items, w) => {
    ...
    for(let i = 0; i < (items.length + 1); i++) { // обход по каждой строке, i - строка
        for (let j = 0; j < (w + 1); j++) { // обход по каждому киллограмму, j - столбец
            ...
            else {  // в противном случае обновляем максимальным значением из предыдщей строки и суммой,
                    // которую составляют стоимость текущего предмета и максимальная цена незанятого веса
              const prev = b[i-1][j]
              const next = (items[i - 1].p) + (b[i-1]?.[j - items[i-1].w])
